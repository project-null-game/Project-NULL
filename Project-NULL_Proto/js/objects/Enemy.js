class Enemy extends Phaser.Physics.Arcade.Sprite {
  ...
  update(player) {
    ...
    if (this.type === 'stick') {
      // 나무 막대기 청년: 바라보는 방향 앞줄 3칸(키패드 형태) 스윙
      const tiles = this.getKeypadTiles(myTx, myTy);
      const hit = tiles.some(([tx, ty]) => tx === pTx && ty === pTy);
      if (hit) this.performAttack(player, 'stick');
    } else {
      // 돌멩이 청년: 같은 행/열 + 사거리 5칸 이내면 직선 투척
      ...
      if ((sameRow || sameCol) && dist > 0 && dist <= this.attackRange) {
        this.performAttack(player, 'rock', sameRow ? 'row' : 'col', myTx, myTy, pTx, pTy);
      }
    }
  }
